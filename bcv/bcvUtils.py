...
Utility functions for veto codes
...
def save_db(cursor, table_name, filename, working_filename, order_by = None, verbose=True):
    ...
    * .bd - sqlite database
    ...
    if filename == '':
        raise ValueError, "Error: empty filename."
...
def read_summary(summary_file):
  """
  read a summary file from omegaveto.m and return dictionary
  """
  lines = [line.split(":",1) for line in open(summary_file) if not line.startswith("#")]
  summary = {}
  for l in lines:
    # split to strip white space
    summary[l[0].split()[0]] = l[1].split()[0]
  
  return summary

def textRead(test_file):
  """
  Read text file like matlab load.
  """
  return zip(*[map(float,line.split()) for line in open(test_file)\
              if (line.split() != [] and not line.startswith("%") and\
              not line.startswith("#"))])
