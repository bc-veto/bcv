function tiling = xtile(timeRange, integrationTime, frequencyRange, sampleFrequency, ...
                        maximumMismatch, highPassCutoff, lowPassCutoff, ...
                        whiteningDuration, transientFactor)
% XTILE Tile space of time and frequency for discrete timefrequency analysis
%
% XTILE is currently being forked from WTILE
%
...
%
% usage: tiling = xtile(timeRange, integrationTime, frequencyRange, sampleFrequency, ...
%                       maximumMismatch, highPassCutoff, lowPassCutoff, ...
%                       whiteningDuration, transientFactor);
%
%  timeRange          duration of analysis
%  integrationTime    time domain length of integration window
...
% $Id: xtile.m 656 2008-04-16 18:48:55Z jrollins $
...
% check for scalar integration time
...
% check for a valid integration time
if integrationTime < 0,
    error('negative integration time');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                        determine resolveable frequencies
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% define the width of the frequency bins
xBandwidth = 1 / integrationTime;

% define the frequencies of the bins
xFrequencies = (0 : ((sampleFrequency * integrationTime) * 0.5)) * xBandwidth;

% constrain the frequency bins to lie in the allowed band
xFrequencies = xFrequencies((xFrequencies > minimumFrequency) & (xFrequencies < maximumFrequency));

% compute the Q of each bin
qs = xFrequencies / xBandwidth;

% define the Q range
minimumQ = min(qs);
maximumQ = max(qs);
qRange = [minimumQ maximumQ];
...
for plane = 1 : numberOfPlanes,
  ...
  if maximumFrequency > maximumAllowableFrequency,
    warning('maximum frequency greater than %.2f', maximumAllowableFrequency);
  end
  ...
  frequencies = xFrequencies(plane);
  ...
  for row = 1 : numberOfRows,
    ...
    numberOfTiles = 2*2^nextpow2(timeRange / integrationTime);
    ...
    halfWindowLength = (timeRange / integrationTime) / 2 - 1
