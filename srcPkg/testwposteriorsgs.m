...
[logSkymap, logGlitches] = wposteriorsgs(channelNames, rate, wSw, xSw, directions, taus);
...
logGlitch = sum(logGlitches)

logOdds = logSignal - logGlitch

probSignal = 1 / (1 + exp(logGlitch - logSignal) + exp(-logSignal))
probGlitch = 1 / (exp(logSignal - logGlitch) + 1 + exp(-logGlitch))
probNoise = 1 / (exp(logSignal) + exp(logGlitch) + 1)
